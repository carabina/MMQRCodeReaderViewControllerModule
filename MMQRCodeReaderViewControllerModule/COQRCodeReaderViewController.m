//
//  COQRCodeReaderViewController.m
...
#import "COQRCodeReaderViewController.h"
#import <Masonry/Masonry.h>
#import <MMUIKitAdditions/UIScreen+MMAdditions.h>
#import <MMUIKitAdditions/UIApplication+MMAdditions.h>
#import <MMUIKitAdditions/UIAlertView+MMAdditions.h>
#import "COScanBGView.h"

@interface COQRCodeReaderViewController () <AVCaptureMetadataOutputObjectsDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate>

@property (nonatomic, strong) COScanBGView *myScanBGView;
...
@implementation COQRCodeReaderViewController
...
#pragma mark - Views
- (void)configUI {
    CGFloat width = [UIScreen mm_width] *2/3;
    CGFloat padding = ( [UIScreen mm_width] - width)/2;
    CGRect scanRect = CGRectMake(padding,  [UIScreen mm_height]/10, width, width);
    ...
    if (!_videoPreviewLayer) {
        ...
        if (!input) {
            [UIAlertView mm_showAlertTip:error.localizedDescription];
            ...
            [captureMetadataOutput setMetadataObjectsDelegate:self queue:dispatch_queue_create("mm_capture_queue",NULL)];
            ...
            if (![captureMetadataOutput.availableMetadataObjectTypes containsObject:AVMetadataObjectTypeQRCode]) {
                [UIAlertView mm_showAlertTip:@"摄像头不支持扫描二维码！"];
                [self.navigationController popViewControllerAnimated:YES];
            } else {
    ...
    if (!_myScanBGView) {
        _myScanBGView = [[COScanBGView alloc] initWithFrame:self.view.bounds];
...
- (void)clickRightBarButton:(UIBarButtonItem *)item {
    if (![UIApplication mm_checkPhotoLibraryAuthorizationStatus]) {
        return;
    }
